...
The physical Kratos run must produce a public probe report showing the portable first-runtime prerequisites defined by ADR-0038 and the merged public development-host contract, including:
...
The public report must have `portable_capabilities_passed: true` and empty probe/repository failure sets while still refusing to claim physical-host acceptance or runtime authorization.
...
3. Use Oracle MCP/RPC to execute the exact merged probe on Kratos's current system.
4. Retain the public probe report plus the Oracle MCP/RPC execution receipt privately as one P01D evidence set.
