# Campaign 001 — AF03 Acceptance

Status: ACCEPTED EVIDENCE RECORD — operative merge binding pending

Recorded: 2026-07-23

## Candidate evidence

- exact candidate head: `4916f79ff3a0cbac8ee4ae53f9ac09a0065d7b7d`;
- AF03 workflow run: `30003085272`;
- retained artifact: `8561809711`;
- artifact digest: `sha256:fa36506e788958d4b3d134f6f1286e4c2d47d309aa15529e6f1d384c9bdcd6a6`;
- validation report SHA-256: `e437e8785a846710e2e2434fb3e537a3566fc6baf68b420da6610dd20571f302`;
- adversarial regressions: `36`;
- exact durable candidate file boundary: `24`;
- candidate merge: `d86218e1127c57bacfb4d88eff15b81d326995ba`.

## Independent Sergeant review

- frozen evidence head reviewed: `a6a1d9aa13f619c2f8ff4c1c6c0cadea331df3d6`;
- result: `pass_with_mandatory_retained_restrictions`;
- blocking findings: `0`;
- review artifact: `archive/campaign-001/af03/SERGEANT-REVIEW.md`.

Sergeant's review is independent evidence. It does not supply Ptah authority and did not itself accept AF03.

## Proposed acceptance effect

- AF03 status: `ACCEPTED COMPLETE`;
- accepted AF03 archive records: `10`;
- blocked AF03 outcomes: `0`;
- AF03 remaining evidence: `0`;
- campaign accepted archive records: `29`;
- campaign blocked completed outcomes: `1`;
- campaign completed formations: `3`;
- AF04 status: `READY / NOT STARTED`.

## Mandatory retained restrictions

Acceptance preserves every record-level restriction, including:

- profile catalogues do not grant source reuse or prove linked private capabilities;
- Semgrep's LGPL-2.1 and proprietary-platform separation;
- Coder's AGPL-3.0 and Premium/provider separation;
- hosted browser/model/service capabilities remain separate from public source;
- minicap remains legacy, private-API and exact-device/SDK bounded;
- Firecracker requires exact KVM and host-security proof;
- Docker CLI remains a client, not Docker Engine or a Ptah authority;
- all donor identities remain aliases and all backend success requires separate Ptah evidence or review.

## Preserved authority

- AF04 remains unstarted;
- Phase 0A remains frozen;
- P01 remains the active implementation-authorization work;
- ADR-0033 remains proposed;
- runtime implementation remains unauthorized.

This record accepts the reviewed AF03 candidate subject to permanent exact-head accepted-state proof. The operative merge is bound in a later recovery-only update.