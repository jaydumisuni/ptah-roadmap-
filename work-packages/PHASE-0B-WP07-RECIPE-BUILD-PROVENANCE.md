...
Catalog `0.1.1` supersedes the incomplete catalog `0.1.0` without mutating its candidate history.
...
- `build.recipe.lifecycle`;
- `build.run.lifecycle`;
- `build.step.lifecycle`;
- `build.cache_record.lifecycle`;
- `build.secret_access.lifecycle`;
- `provenance.trust_policy.lifecycle`;
- `provenance.verification_run.lifecycle`;
- `provenance.reproduction_request.lifecycle`;
- `proof.reproduction_run.lifecycle`.
...
1. Recipe, immutable Revision, Proposal, Acceptance, Readiness, Backend Compatibility, Plan and Run remain separate.
...
4. Readiness is exact-context and expiring.
5. Backend Compatibility is directional and cannot silently weaken required semantics.
6. Compiled Plan never mutates Recipe Revision and remains backend-specific/replaceable.
7. `build.step` is the run-scoped Step record; embedded Recipe `step_key` is the immutable definition key. The redundant draft `build.step_run` token is removed.

### Execution

1. Build Run binds exact Recipe/Acceptance/Readiness/Plan/Workspace Materialization/Provider generation.
2. local and remote Provider locality remain truthful.
...
5. cleanup failure remains explicit and restricted.
...
3. paths/tags/coordinates/URLs remain Aliases or Locations.
4. produced bytes become WP03 Objects before Artifact promotion.
5. Artifact Release remains a separate allowlisted authority record.
6. output presence or upload never means release accepted.
...
5. offline/no-log verification remains supported without fabricated log evidence.
...
- stale readiness or mutable alias promoted to exact material;
...
- secret references, cleanup and leakage boundaries;
- output registration, mutable tags and release authority;
...
Structural JSON Schema validation is insufficient. WP13 must enforce typed-reference kinds, cross-record equality, generation/epoch, graph acyclicity/dependencies, mutable-material resolution, cache identity, secret redaction, output/digest linkage, coverage, trust-policy semantics and reproduction independence.
